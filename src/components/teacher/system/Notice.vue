<template>
  <v-container>
    <v-container>
      <form>
        <v-card class="mx-auto" max-width="70%" color outlined>
          ...
          <v-card-text>
            ...
            <div>
              <v-btn @click.native="selectFile" v-if="!uploadEnd && !uploading">
                add photo
                <v-icon right aria-hidden="true"> </v-icon>
              </v-btn>
              <form ref="form">
                <input
                  id="files"
              ...
              <img v-if="uploadEnd" :src="downloadURL" width="20%" />
              <div v-if="uploadEnd">
                <v-btn
                  class="ma-0"
                  dark
    ...
    <v-container
      dense
      v-for="(message, index) in notices"
      :key="index"
            ...
            <v-responsive>
              ...
              <v-card-actions>
                ...
                <v-btn
                  depressed
                  color="error"
                  @click="deletePost(message.id, index)"
                  >Delete</v-btn
                >
              </v-card-actions>
            </v-responsive>
  ...
  methods: {
    ...
    upload(file) {
      ...
      this.uploadTask = firebase
        .storage()
        .ref("noticeImage/" + file.name)
        .put(file);
    },
    deleteImage() {
      firebase
        .storage()
        .ref("noticeImage/" + this.fileName)
        .delete()
        .then(() => {
          this.uploading = false;
          this.uploadEnd = false;
